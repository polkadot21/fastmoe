...
class Top2Router(nn.Module):
    ...
    def forward(self, x):  # x: [T, D]
        ...
        topv, topi = torch.topk(probs, k=2, dim=-1)
...
###
# Cell 4 — Overlap-correct 2-way microbatch EP MoE
###
...
class RecvPack:
    ...
    gate_w: torch.Tensor
    ev_payload_done: torch.cuda.Event  # signals payload enqueued+completed on its comm stream


class DeepEPMoE(nn.Module):
    """
    Overlap-correct schedule:
      1) dispatch mb0 on stream0
      2) dispatch mb1 on stream1
      3) compute mb0 waits ONLY on mb0 dispatch event
      4) combine mb0 on stream0 overlaps with compute mb1
      5) compute mb1 waits ONLY on mb1 dispatch event
      6) combine mb1
    """

    def __init__(self, d_model: int, d_hidden: int, local_experts: int, ep_group=None):
        super().__init__()
        ...
        # Two independent comm streams (one per microbatch) => avoids false dependencies
        self.dispatch_streams = [torch.cuda.Stream(priority=-1), torch.cuda.Stream(priority=-1)]
        self.combine_streams = [torch.cuda.Stream(priority=-1), torch.cuda.Stream(priority=-1)]

        # Compute and assemble streams
        self.compute_stream = torch.cuda.Stream(priority=0)
        self.assemble_stream = torch.cuda.Stream(priority=0)

    def _dispatch_one_microbatch(self, x_mb: torch.Tensor, mb: int, mb_tag: str):
        """
        Dispatch on stream = dispatch_streams[mb].
        Returns RecvPack that includes a CUDA event ev_payload_done.
        """
        stream = self.dispatch_streams[mb]
        device = x_mb.device
        Tmb, D = x_mb.shape

        topi, topw = self.router(x_mb)

        # Replicate for top-2
        x_rep = x_mb.unsqueeze(1).expand(Tmb, 2, D).reshape(-1, D).contiguous()
        expert_id = topi.reshape(-1).to(torch.int32).contiguous()
        gate_w = topw.reshape(-1).contiguous()
        orig_idx = torch.arange(Tmb, device=device, dtype=torch.int32).repeat_interleave(2)
        ...
        with torch.cuda.stream(stream):
            ...
            # This event will only complete when *all prior ops on this stream* complete.
            # That includes NCCL work if it's correctly stream-ordered by PyTorch.
            ev = torch.cuda.Event()
            ev.record(stream)

        recv_pack = RecvPack(
            x=x_recv, expert_id=e_recv, orig_idx=o_recv, gate_w=g_recv, ev_payload_done=ev
        )
        meta = DispatchMeta(
    ...
    def _expert_compute(self, recv_pack: RecvPack, mb_tag: str):
        """
        Compute waits on recv_pack.ev_payload_done (NOT on the entire dispatch stream).
        This enables overlap: compute mb0 can run while dispatch mb1 is in-flight.
        """
        with torch.cuda.stream(self.compute_stream):
            # Precise dependency: wait only for this microbatch's dispatch event
            self.compute_stream.wait_event(recv_pack.ev_payload_done)
            ...
            with record_function(f"expert_compute_{mb_tag}"):
                ...
                local_e = (expert_id_global - local_base).to(torch.int64)
    ...
    def _combine_and_assemble(
        self, y_recv: torch.Tensor, meta: DispatchMeta, Tmb: int, mb: int, mb_tag: str
    ):
        """
        Combine on combine_streams[mb], assemble on assemble_stream.
        """
        comb_stream = self.combine_streams[mb]

        # Launch return all_to_all on its own comm stream
        with torch.cuda.stream(comb_stream):
            ...
            ev_ret = torch.cuda.Event()
            ev_ret.record(comb_stream)

        # Assemble waits on return event
        with torch.cuda.stream(self.assemble_stream):
            self.assemble_stream.wait_event(ev_ret)
    ...
    def forward(self, x: torch.Tensor):
        ...
        # ---- overlap-correct ordering ----
        # 1) dispatch both microbatches first (on independent streams)
        recv0, meta0 = self._dispatch_one_microbatch(x0, mb=0, mb_tag="mb0")
        recv1, meta1 = self._dispatch_one_microbatch(x1, mb=1, mb_tag="mb1")

        # 2) compute mb0 (wait only on mb0 dispatch)
        y0 = self._expert_compute(recv0, "mb0")

        # 3) start combine+assemble mb0 (can overlap with compute mb1)
        out0 = self._combine_and_assemble(y0, meta0, T0, mb=0, mb_tag="mb0")

        # 4) compute mb1
        y1 = self._expert_compute(recv1, "mb1")

        # 5) combine+assemble mb1
        out1 = self._combine_and_assemble(y1, meta1, T - T0, mb=1, mb_tag="mb1")

        # ensure assemble finished before returning
...
def run_profiled_demo(
    ...
    for i in range(warmup_iters):
        ...
        opt.step()
        torch.cuda.synchronize(device)
        r0log(f"[warmup] iter {i+1}/{warmup_iters} loss={loss.item():.4f}")
    ...
    for step in range(steps):
        ...
        prof.step()
        torch.cuda.synchronize(device)
        r0log(f"[train] step {step+1}/{steps} loss={loss.item():.4f}")
...
def _worker(rank, world_size):
    # Fail-fast + avoid localhost IPv6 issues
